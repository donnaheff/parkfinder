...
module.exports = async (req, res) => handle(async () => {
  ...
  await run(client.from('admin_actions').insert({ admin_id: admin.id, target_type: 'parking_lot', target_id: lot.id, action: 'request-info', notes }));
  const owner = lot.owner_id ? await one('owners', lot.owner_id) : null;
  if (owner?.email) await sendAdminDecisionEmail({ to: owner.email, lotName: lot.name, decision: 'request-info', notes });
  ok(res, updated[0]);
}, res);
