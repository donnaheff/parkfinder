...
export default function LotDetailClient({ lot, initialReviews }) {
  ...
  useEffect(() => {
    if (!session) return;
    let cancelled = false;
    getSavedParks().then((rows) => { if (!cancelled) setSaved(rows.some((r) => r.id === lot.id)); }).catch(() => {});
    return () => { cancelled = true; };
  }, [session, lot.id]);

  async function reloadReviews() {
    try {
      setReviews(await getReviews(lot.id));
    } catch {
      // Reviews are a secondary feature — keep showing the last known list.
    }
  }
